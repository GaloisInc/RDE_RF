...
object SysMLParser extends Parser {
  ...
  def parseLine(line: String): Option[ParsedReference] = {
    ...
    cleanLine(line) match {
      ...
      case systemRegex(acronym, name) => Some(ParsedRef(emptyIfNull(name), line, ReferenceType.System, emptyIfNull(acronym)))
      case componentRegex(acronym, name, symbol, parent) => Some(ParsedRefWithReferences(emptyIfNull(name), line, ReferenceType.Component, emptyIfNull(acronym), symbol, parent))
      case subsystemRegex(acronym, name, symbol, parent) => Some(ParsedRefWithReferences(emptyIfNull(name), line, ReferenceType.SubSystem, emptyIfNull(acronym), symbol, parent))
      case attributeRegex(acronym, name, symbol, parent) => Some(ParsedRefWithReferences(emptyIfNull(name), line, ReferenceType.Attribute, emptyIfNull(acronym), symbol, parent))
      case requirementRegex(acronym, name, symbol, parent) => Some(ParsedRefWithReferences(emptyIfNull(name), line, ReferenceType.Requirement, emptyIfNull(acronym), symbol, parent))
      case actionRegex(acronym, name, symbol, parent) => Some(ParsedRefWithReferences(emptyIfNull(name), line, ReferenceType.Event, emptyIfNull(acronym), symbol, parent))
      case usecaseRegex(acronym, name, symbol, parent) => Some(ParsedRefWithReferences(emptyIfNull(name), line, ReferenceType.Scenario, emptyIfNull(acronym), symbol, parent))
      case importRegex(name) => Some(ParsedRef(emptyIfNull(name), line, ReferenceType.Import, ""))
      case viewRegex(name, parent) => Some(ParsedRefWithReferences(emptyIfNull(name), line, ReferenceType.View, "", ":", parent))
      case viewPointRegex(name) => Some(ParsedRef(emptyIfNull(name), line, ReferenceType.ViewPoint, ""))
      case _ => None
    }
  }
