...
class SysMLAnalyzerTest extends AnyFlatSpec with should.Matchers {
  ...
  "SysMLReader" should "to extract parts and items" in {
    ...
    testUtility.checkExtractReferences("RTS_Glossary", documentEnricher, expectedDocumentType, filePath, numberExprectedSystem = 1, numberOfSubSystem = 80, numberOfComponent = 66)
  }
  ...
  "SysMLReader" should "to extract Scenarios" in {
    ...
    val filePath = getPathToDocument(fileName, resourceFolder)
    testUtility.checkExtractReferences(fileName, documentEnricher, expectedDocumentType, filePath, 4, 0, 1, numberOfScenarios = 39)
  }

  "SysMLReader" should "to extract Views" in {
    val fileName = "RTS_Viewpoints"
    val filePath = getPathToDocument(fileName, resourceFolder)
    testUtility.checkExtractReferences(fileName, documentEnricher, expectedDocumentType, filePath, numberExprectedSystem = 1, numberOfViews = 8)
  }

  "SysMLReader" should "to extract Actions" in {
    val fileName = "RTS_Actions"
    val filePath = getPathToDocument(fileName, resourceFolder)
    testUtility.checkExtractReferences(fileName, documentEnricher, expectedDocumentType, filePath, numberExprectedSystem = 4, numberOfEvents = 20)
  }
