...
class SysMLDocumentEnricher(override val formatterType: LatexFormatter,
  ...
  def transformReference(line: String, fileName: String): Option[DocReference] = {
    ...
    val extractedReference = cleanString(line) match
      case systemRegex(acronym, name) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), noneIfNull(acronym)), ReferenceType.System, DocumentType.SysML, line))
      case componentRegex(acronym, name, symbol, references) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), noneIfNull(acronym)), ReferenceType.Component, DocumentType.SysML, line, references = refinementRefs(symbol, references)))
      case subsystemRegex(acronym, name, symbol, references) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), noneIfNull(acronym)), ReferenceType.SubSystem, DocumentType.SysML, line, references = refinementRefs(symbol, references)))
      case attributeRegex(acronym, name, symbol, references) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), noneIfNull(acronym)), ReferenceType.Attribute, DocumentType.SysML, line, references = refinementRefs(symbol, references)))
      case requirementRegex(acronym, name, symbol, references) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), noneIfNull(acronym)), ReferenceType.Requirement, DocumentType.SysML, line, references = refinementRefs(symbol, references)))
      case actionRegex(acronym, name, symbol, references) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), noneIfNull(acronym)), ReferenceType.Event, DocumentType.SysML, line, references = refinementRefs(symbol, references)))
      case importRegex(name) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), None), ReferenceType.Import, DocumentType.SysML, line))
      case viewRegex(name, references) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), None), ReferenceType.View, DocumentType.SysML, line, references = refinementRefs(":", references)))
      case viewPointRegex(name) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), None), ReferenceType.ViewPoint, DocumentType.SysML, line))
      case usecaseRegex(acronym, name, symbol, references) => Some(DocReference(fileName, ReferenceName(emptyIfNull(name), noneIfNull(acronym)), ReferenceType.Scenario, DocumentType.SysML, line, references = refinementRefs(symbol, references)))
