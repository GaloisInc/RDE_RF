...
class ObjectConfigGeneratorTest extends AnyFlatSpec with should.Matchers {
    ...
    "ObjectConfigGenerator" should "generate a config object" in {
      ...
      val report = ReportReference(title, folder, Array(documentInfo), Array.empty, Array.empty, PaperLayout.A4)

      val reportFilePath = ObjectConfigGenerator.generate(report)
