...
class ObjectConfigGeneratorTest extends AnyFlatSpec with should.Matchers {
  ...
  lazy val getTestFile: Array[String] = {
    ...
    val FRETDocumentInfo = getClass.getResource("../Fret").getPath
    ...
    val filesToAnalyze = FileUtil.getFilesInDirectory(sysmlDocuments).toArray ++
      ...
      FileUtil.getFilesInDirectory(FRETDocumentInfo).toArray
    filesToAnalyze
  }
  ...
  "ObjectConfigGenerator" should "generate a config object" in {
    ...
    val report = ReportReference(title, folder, Array.empty, Array.empty,  Array.empty,Array.empty, Array.empty, Array.empty, Array.empty, Array.empty,Array.empty, PaperLayout.A4)
  ...
  "ObjectConfigGenerator" should "generate a config file from documents" in {
    ...
    val filesToAnalyze = getTestFile
